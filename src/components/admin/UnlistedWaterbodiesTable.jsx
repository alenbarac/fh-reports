...
import { CCol, CContainer, CRow, CCard, CCardHeader, CCardBody, CBadge, CButton, CAlert, CForm, CFormInput, CFormLabel, CFormTextarea, CModal, CModalBody, CModalFooter, CModalHeader, CFormCheck, CToast, CToastHeader, CToastBody, CModalTitle, CFormSelect, CSpinner } from '@coreui/react';
...
import { DataTypeProvider, IntegratedFiltering, SearchState, PagingState, IntegratedPaging, EditingState } from '@devexpress/dx-react-grid';
...
const Popup = ({ row, onChange, onApplyChanges, onCancelChanges, open }) => {
  const { regionCollection } = useGlobalContext();
  return (
            ...
            <CRow className='align-items-center'>
              <CCol md={12}>
                ...
                >
                  ...
                  {regionCollection?.map((region) => <option key={region.id} value={region.id} selected={region.id === row.regionId ? 'selected' : ''}>{region.region_name}</option>)}
...
const UnlistedWaterbodiesTable = ({ user }) => {
  ...
  const { loading, getAllRegions } = useGlobalContext();
  ...
  const getTableData = async () => {
    ...
    await apiClient
      .get(`/admin/waterbodies_unlisted`)
      .then((response) => {
                  ...
                  <CRow>
                    <CCol>
                      <CCard className='mb-4'>
                        <CCardHeader className='card-header__darkblue'>Unlisted Waterbodies  </CCardHeader>
...
export default UnlistedWaterbodiesTable